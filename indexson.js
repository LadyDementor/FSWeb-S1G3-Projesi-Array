...
function cesitEkle(tat2){
  return orijinalTatlar.unshift(tat2);
}
cesitEkle('Kakule');
...
console.log(indekstekiCesitiGetir(3));
...
*/
let tatlar = [...orijinalTatlar];


function ismeGoreCesitCikar(alerjen){
return tatlar.splice(alerjen, 1)
}

function sirabul(gida){
  return tatlar.indexOf(gida)
}
ismeGoreCesitCikar(sirabul('Tarçın'));
console.log(tatlar);
...
function ortalamaKelimeSayisi(ortalamadizi){
  console.log("baha avg", ortalamadizi.length);
  /*kod buraya*/
...
function dizi25cesitmi(kartonkolikutu){
  ...
  console.log("25 mi?",kartonkolikutu.length);
}
...
const toplamtatlar = [...orijinalTatlar, ...yeniTatlar, ...mevsimlikTatlar, ...bolgeselTatlar]

function sonfonk(son){
  for (let i=0; i<29; i++) {
   son.splice(Math.floor((Math.random()) * (54-i)), 1)
  }
  return son
}
console.log(sonfonk(toplamtatlar));
